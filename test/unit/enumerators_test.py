...
@pytest.fixture(params=[k, w1, w2])
def weights(request):
    return request.param


def test_enumerator(enumerator, support, weights):
    """
    ...
    enumerator_instance = enumerator(support, weights, env)
    truth_assignments = list(enumerator_instance.enumerate(smt.Bool(True)))
    ...
    for ta, _ in truth_assignments:
        ...
        assert enumerator_instance.weights.weight_from_assignment(
            ta
        ).is_constant(), (
