...
def random_cnf(reals, bools, n_clauses, len_clauses, seed=None, bounds=[0,1], p_bool=0.5):
    ...
    for _ in range(n_clauses):
        ...
        for _ in range(len_clauses):
            if n_bools > 0 and np.random.random() < p_bool: # sampling a Boolean atom
                ...
                atom = op(Plus(*[Times(Real(float(A[i])), reals[i])
...
def random_weight(reals, bools, depth, seed=None,
                  vbounds=[0,1], dbounds=[0,4], cbounds=[-10,10],
                  max_monomials=10, p_bool=0.5):
    ...
    if depth == 0:
        ...
        condition = random_cnf(reals, bools, 1, 1,
                               bounds=vbounds, p_bool=p_bool)
        left = random_weight(reals, bools, depth - 1,
                             vbounds=vbounds, dbounds=dbounds,
                              max_monomials=max_monomials, p_bool=p_bool)
        right = random_weight(reals, bools, depth - 1,
                              vbounds=vbounds, dbounds=dbounds,
                              cbounds=cbounds,
                              max_monomials=max_monomials, p_bool=p_bool)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dbounds', type=int, nargs=2, help="Bounds on degree", default=(0,3))
    ...
    f = random_cnf(reals, bools, args.n_clauses, args.len_clauses,
                   seed=args.seed, bounds=args.vbounds, p_bool=args.p_bool)
    w = random_weight(reals, bools, args.depth, seed=args.seed,
                      vbounds=args.vbounds, dbounds=args.dbounds,
