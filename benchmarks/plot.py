...
def error_plots(exact_results, approx_results, args):
    ...
    for m in args.methods:
        ...
        if len(classes) == 1:
            ...
            ax.fill_between(
                x,
                np.max([np.zeros(len(y)), y - ystd], axis=0),
                y + ystd,
                alpha=ALPHA / 2,
            )
...
def runtime_scatter(xresults, yresults, args):
    ...
    for f in all_filenames:
        ...
        if r1 is not None and (tmin is None or r1 < tmin):
            tmin = r1
        if r2 is not None and (tmin is None or r2 < tmin):
            tmin = r2
        if r1 is not None and (tmax is None or r1 > tmax):
            tmax = r1
        if r2 is not None and (tmax is None or r2 > tmax):
            tmax = r2
    ...
    # timeout lines
    ax.axhline(
        args.timeout, 0, args.timeout, linestyle="--", color=TIMEOUT_COLOR
    )  # timeout lines and label
    ...
    for i, cl in enumerate(sorted_classes):
        ax.scatter(
            xtime[cl], ytime[cl], marker=MARKER, alpha=ALPHA, color=colors[i], label=cl
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_directory", type=str, help="Plot directory", default="."
    )
    parser.add_argument(
        "--logscale", action="store_true", default=False, help="Use logscale"
    )
    parser.add_argument(
        "--interactive", action="store_true", default=False, help="Use logscale"
    )
    ...
    ts_parser.add_argument(
        "--xpaths", type=str, nargs="+", default=[], help="Paths to Method 1's results"
    )
    ts_parser.add_argument(
        "--ypaths", type=str, nargs="+", default=[], help="Paths to Method 2's results"
    )
    ts_parser.add_argument("--timeout", type=int, help="Timeout")
    ts_parser.add_argument(
        "--classes", type=str, help="Partition classes by filename substring"
    )
    ...
    err_parser.add_argument(
        ...
        nargs="+",
        default=[],
        help="Directories with approximate results",
    )
    err_parser.add_argument(
        "--methods", type=str, nargs="+", default=[], help="Approx. methods' names"
    )
    err_parser.add_argument(
        "--classes", type=str, help="Partition classes by filename substring"
    )
    err_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Partition classes by filename substring",
    )
    ...
    if args.plot == "error":
        exact_results = parse_results(
            map(lambda f: os.path.join(args.exact, f), os.listdir(args.exact))
        )
        ...
        for i, m in enumerate(args.methods):
            approx_results[m] = parse_results(
                map(
                    lambda f: os.path.join(args.approx[i], f),
                    os.listdir(args.approx[i]),
                )
            )
