if __name__ == '__main__':
    ...
    for f in input_files:
        with open(f) as json_file:
            ...
            if mode not in results[p_key]:
                # [sum_time, sum_int, n_elements]
                results[p_key][mode] = [0, 0, 0];
            
            for res in data["results"]:
                results[p_key][mode][0] += res["time"]
                results[p_key][mode][1] += res["n_integrations"]
                results[p_key][mode][2] += 1
    ...
    for key in results:
        for mode in results[key]:
            ...
            results[key][mode] = [res[0]/res[2], res[1]/res[2]]
            ...
            if len(x_values) > 1:
                ...
                for x in x_values:
                    ...
                    for mode in res_modes:
                        res_time, res_integrations = res_modes[mode]
                        ...
                        plot_axis[mode]["time"].append(res_time)
                        plot_axis[mode]["integrations"].append(res_integrations)
