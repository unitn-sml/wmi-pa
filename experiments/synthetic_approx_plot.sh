...
# fix nreals to 3
python3 plot_approx.py $RES_DIR/ --output $PLT_DIR/ --filename "_e0.1_nreals_3" -o $PLT_DIR/ --fix integrator_error=0.1 nreals=3 --rows-var pd --cols-var integrator_N
python3 plot_approx.py $RES_DIR/ --output $PLT_DIR/ --filename "_e0.1_degree_4" -o $PLT_DIR/ --fix integrator_error=0.1 pd=4 --rows-var nreals --cols-var integrator_N
