...
class Monomial:
    ...
    def __init__(self, expression):
        ...
        """
        if not (expression.is_times() or expression.is_real_constant() or expression.is_symbol() or is_pow(expression)):
...
class Polynomial:
    ...
    def _preprocess_formula(expression, aliases):
        ...
        if len(aliases) > 0:
            ...
            try:
                sorted_substitutions = [node for node in nx.topological_sort(Gsub) if node in aliases]
...
class Bound:
    ...
    def __init__(self, expression, aliases={}):
        ...
        if right.is_real_constant():
            ...
            self._parse_expression(Plus(left, Times(Real(-1), right)), Real(0), False, aliases)
    ...
    def _parse_expression(self, polynomial, constant, negate, aliases):
        ...
        if poly.degree() > 0:
            ...
            denominators = [m.coefficient.denominator for m in poly.monomials] + [b.denominator]
            lcd = lcmm(denominators)
            for monomial in poly.monomials:
                assert (
                    len(monomial.exponents) == 1 and list(monomial.exponents.values())[0] == 1
